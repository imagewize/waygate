...
### Added
- Six built-in prompt templates (Homepage, About, Services, Contact, Landing Page, Portfolio) in `AiIntegration::get_prompt_templates()`; filterable via the `waygate_prompt_templates` hook so third-party themes/plugins can add or remove templates
